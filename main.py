...
def install_chrome_driver_hidden():
    ...
    try:
        ...
        st.error("Could not detect Chrome version. Please make sure Chrome is installed.")
        raise
    ...
    try:
        ...
        st.error(f"Failed to fetch ChromeDriver version for Chrome {chrome_version}.")
        raise
    ...
    try:
        ...
        st.error(f"Could not download ChromeDriver from {zip_url}")
        raise

    # Extract chromedriver binary (handles nested folders)
    with zipfile.ZipFile(zip_path, "r") as zip_ref:
        for file in zip_ref.namelist():
            if file.endswith(driver_name):
                extracted_path = zip_ref.extract(file, base_driver_dir)
                # Move/overwrite to target driver path
                if os.path.exists(driver_path):
                    os.remove(driver_path)
                os.replace(extracted_path, driver_path)
                break
